{lib, config, pkgs, modulesPath, buildUBoot, armTrustedFirmwareRK3588, rkbin, ...}:
let
...
in {
  ...
  sdImage = {
    ...
    postBuildCommands = ''
      ...
      dd if=${uboot}/u-boot-rockchip.bin of=$img seek=64 conv=fsync,notrunc
    '';
  };
}
